...
describe('CLI', () => {
  ...
  describe('#asciiGreeting()', () => {
    it('should log the greeting message', () => { 
      cli = new CLI(serverless, []);
      cli.consoleLog = sinon.stub();
      const processedInput = cli.asciiGreeting();

      expect(cli.consoleLog.firstCall.args[0]).to.contain('The Serverless Application Framework');
    });
  });

  describe('#displayHelp()', () => {
    it('should call consoleLog', () => { 
      cli = new CLI(serverless, []);
      cli.consoleLog = sinon.stub().returns();
      const processedInput = cli.processInput();
      cli.displayHelp(processedInput);

      expect(cli.consoleLog.called).to.equal(true);
    });
    ...
    it('should return false if no "help" or "version" related command / option is given', () => {
      ...
      console.log('cli.consoleLog', cli.consoleLog);
    });
  });
